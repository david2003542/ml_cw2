...
def local_energy(i, j, visable_image, hidden_image):# h* \sum(x_i) -beta* \sum(x_i x_j) -eta \sum(x_i y_i)
    ...
    neighbours = get_neighbours(i, j, m, n, 8)
    total_pixels = m*n
    h = const_list[0]
    beta = const_list[1]
    eta = const_list[2]
...
def calculate_total_energy(visable_image, hidden_image):
    global m, n 
    energy = 0.
    for i in range(m):
        for j in range(n):
            energy += local_energy(i, j, visable_image, hidden_image)
    return energy


def icm(i, j, total_energy, visable_image, hidden_image):
    energy = local_energy(i, j, visable_image, hidden_image)
    other_energy = total_energy - energy
    # print("other", other_energy)
    #flip
    temp_hidden_image = np.copy(hidden_image)
    if(temp_hidden_image[i,j]>1):
        temp_hidden_image[i,j] = 0
    else:
        temp_hidden_image[i,j] = 1
    temp_energy = local_energy(i, j, visable_image, temp_hidden_image)
    # print("energy", energy)
    # print("temp_energy", temp_energy)
    if energy > temp_energy:
        print("flip")
        if(hidden_image[i,j]>1):
            hidden_image[i,j] = 0
        else:
            hidden_image[i,j] = 1
        flipped_energy = local_energy(i, j, visable_image, hidden_image)  
...
total_energy = calculate_total_energy(image, hidden_image)
# print("init", total_energy)
for times in range(1):
    for i in range(2):
        for j in range(n):
            icm(i, j, total_energy, image, hidden_image)
            # print(total_energy)
